...
#                                                                             
# PROGRAMMER: Rahul Saini
# DATE CREATED: 25/05/2019
# REVISED DATE: 29/05/2019
...
def print_results(results_dic, results_stats_dic, model, 
    ...
    if print_incorrect_dogs == True:
        if (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']):
            print("Below is the list of misclassfied dogs : image and then the classfier label")
            for image,result in results_dic.items():
                if (sum(result[3:]) == 1):
                    print(image,result[1]) 
        else:
            print("There are no misclassfied dogs")
    if print_incorrect_breed == True:
        if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breeds']:
            print("Below is the list of misclassfied breeds : image and then the classfier label")
            for image,result in results_dic.items():
                if (sum(result[3:]) == 2 and result[2] == 0):
                    print(image,result[1]) 
        else:
            print("There are no misclassfied breeds")
